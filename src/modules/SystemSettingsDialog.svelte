<script lang="ts">
  ...
  export let platformCapabilities: PlatformCapabilities = {
    isIos: false,
    canRequestFullscreen: true,
    showMiuiInstallHint: true,
  }
...
>
  <div class="grid gap-2">
    <ActionRow
      {theme}
      title="添加到主屏幕"
      description={platformCapabilities.isIos
        ? '用 Safari 分享菜单添加到主屏幕'
        : canInstallPwa
          ? '点击发起添加；失败时先开启桌面快捷方式权限'
          : '浏览器菜单添加；小米/MIUI 需开启桌面快捷方式权限'}
      tone="cyan"
    ...
    {#if platformCapabilities.showMiuiInstallHint}
      <div
    ...
    {#if platformCapabilities.canRequestFullscreen}
      <ActionRow
        {theme}
